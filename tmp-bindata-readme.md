...
- ACTIVITY_FLOWER_TASK_CONF.json：活动鲜花任务的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 46 条记录；首记录字段：id 、 task_type 、 task_conf_id 、 reward_type 、 reward_id。
...
- ACTIVITY_MAINTAB_CONF.json：活动主页签的名称、图标与入口展示配置。 结构：RocoDataRows + LocalizationStrings；约 3 条记录；首记录字段：id 、 maintab_name 、 maintab_icon_select 、 maintab_icon。
...
- ACTIVITY_PET_RAISE_TASK_CONF.json：活动宠物养成任务的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 12 条记录；首记录字段：id 、 editor_name 、 task_type_name 、 task_type 、 final_task_id 、 task_id 、 icon_normal 、 icon_selected。
...
- ACTIVITY_RELAY_PAGE.json：活动或赛季玩法中的活动接力页面相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 1 条记录；首记录字段：id 、 select_des 、 relay_part_group。
- ACTIVITY_REWARD_BY_STAGE_CONF.json：活动奖励按阶段的奖励内容、奖励档位或发放条件配置。 结构：RocoDataRows + LocalizationStrings；约 15 条记录；首记录字段：id 、 start_time 、 end_time 、 reward_group。
...
- ACTIVITY_TASK_GO_CONF.json：活动任务GO的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 52 条记录；首记录字段：id 、 task_name 、 is_appear_if_finish。
- ACTIVITY_TLOG_CONF.json：活动或赛季玩法中的活动埋点相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 20 条记录；首记录字段：id 、 activity_id 、 base_id 、 partake_seq_num 、 complete_seq_num 、 partake_prefix 、 complete_prefix 、 partake_event。
...
- AONE_FINAL_BATTLE_PETSLIST_CONF.json：AONEFINAL战斗PETSLIST的战斗规则、技能效果、属性克制或数值判定配置。 结构：RocoDataRows + LocalizationStrings；约 12 条记录；首记录字段：ID 、 monster_id 、 called_dialogue_id 、 keyword。
- BATTLE_PASS_CONF.json：战斗通行证的赛季周期、等级上限、主题与开放时间配置。 结构：RocoDataRows + LocalizationStrings；约 1 条记录；首记录字段：id 、 bp_name 、 theme_id 、 top_level 、 loop_level 、 level_name 、 open_time 、 close_time。
- BATTLE_PASS_GIFT_CONF.json：战斗通行证的礼包档位、性别资源与赠礼物品配置。 结构：RocoDataRows + LocalizationStrings；约 12 条记录；首记录字段：id 、 editor_name 、 editor_gender 、 gender 、 bp_id 、 bp_theme 、 bp_grade 、 gift_goods_id。
- BATTLE_PASS_REWARD_CONF.json：战斗通行证的等级奖励、经验需求与奖励组配置。 结构：RocoDataRows + LocalizationStrings；约 200 条记录；首记录字段：id 、 bp_level 、 belong_reward_set_id 、 bp_id 、 bp_theme_id 、 editor_name 、 need_exp 、 male_free_reward_id。
- BATTLE_PASS_TASK_MODULE_CONF.json：战斗通行证任务模块的权重、模块归属与任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 6 条记录；首记录字段：id 、 editor_name 、 task_weight 、 moduel_id。
- BATTLE_PASS_THEME_CONF.json：战斗通行证的主题包装、主题宠物与特殊技能展示配置。 结构：RocoDataRows + LocalizationStrings；约 2 条记录；首记录字段：id 、 theme_name 、 theme_desc 、 theme_petbase_id 、 is_alter 、 theme_pet_title 、 theme_art_set 、 special_skills。
- BATTLE_PASS_UI_COLOR.json：战斗通行证界面组件的主题色、控件配色与视觉风格配置。 结构：RocoDataRows + LocalizationStrings；约 39 条记录；首记录字段：id 、 editor_name 、 umg_name 、 widget_name 、 color_group。
- BONUS_EVENT_ACCU_POOL_CONF.json：活动或赛季玩法中的奖励事件累计池相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 38 条记录；首记录字段：id 、 accumulate_type 、 priority 、 max_count 、 accu_times 、 trig_times 、 trig_prob。
- BONUS_EVENT_PETLIST_CONF.json：活动或赛季玩法中的奖励事件PETLIST相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 350 条记录；首记录字段：id 、 inject_id 、 action_bonus_id 、 petbase_id。
- BONUS_EVENT_POOL_CONF.json：活动或赛季玩法中的奖励事件池相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 7230 条记录；首记录字段：id 、 accumulate_type 、 enabletime 、 disabletime 、 pool_id 、 bonus_type 、 bonus_param 、 weight。
- BONUS_EVENT_PROB_CONF.json：活动或赛季玩法中的奖励事件概率相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 20 条记录；首记录字段：id 、 event_id 、 var_type 、 var_param 、 belong_factor_type。
...
- CHEER_POINT_CONF.json：活动或赛季玩法中的CHEER点数相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 5 条记录；首记录字段：id 、 topic 、 pet_type_1 、 cheer_point。
- DAILY_GLOBAL_CONFIG.json：每日系统的全局参数、默认值与开关配置。 结构：RocoDataRows + LocalizationStrings；约 9 条记录；首记录字段：id 、 key 、 editor_name 、 str。
- DAILY_TASK_REWARD_CONF.json：每日任务的奖励内容、奖励档位或发放条件配置。 结构：RocoDataRows + LocalizationStrings；约 11 条记录；首记录字段：id 、 world_level 、 title 、 task_id。
- EVENT_BASE_CONF.json：活动或赛季玩法中的事件基础相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 64 条记录；首记录字段：id 、 name 、 event_combine 、 level_reward_value 、 coin_reward_value 、 upgrade_reward 、 upgrade_show 、 upgrade_select。
- EVENT_COMBINE_CONF.json：活动或赛季玩法中的事件组合相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 6 条记录；首记录字段：id 、 type 、 num1 、 num2 、 level_reward_value 、 upgrade_reward 、 upgrade_show 、 upgrade_select。
...
- SEASON_TIPS_TAB_CONF.json：活动或赛季玩法中的SEASONTIPS页签相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 1 条记录；首记录字段：id 、 tips_name 、 tips_icon 、 tab_group。
...
- TALE_NOTEBOOK_KELI_CONF.json：活动或赛季玩法中的TALENOTEBOOKKELI相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 4 条记录；首记录字段：id 、 page_unlock_text 、 page_title 、 page_unlock_by_dia 、 list_title 、 page_black_list_unlock_by_dia 、 black_text 、 unlock_medal。
...
- WEEKLY_PHOTO_CONF.json：每周拍照的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 8 条记录；首记录字段：id 、 res_name 、 action。
...
- GUARD_CONF.json：宠物或魔法系统中的守护相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 5 条记录；首记录字段：id 、 is_open 、 Type 、 item_id 、 item_name 、 daily_max_times 、 daily_max_counts 、 reason_white_list。
...
- OWL_SANCTUARY_CONF.json：宠物或魔法系统中的猫头鹰圣所相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 43 条记录；首记录字段：id 、 owl_sanctuary_type 、 init_level 、 level_max 、 slot_num 、 owl_npc_pet_habitat_group_id 、 mix_blood 、 cost_item_id。
...
- PET_EGG_WAY_TO_PROB_CONF.json：宠物蛋WAYTO概率的宠物、孵化、骑乘、魔法或相关玩法配置。 结构：RocoDataRows + LocalizationStrings；约 7 条记录；首记录字段：id 、 egg_shining_prob_type 、 egg_glass_prob_type。
- PET_EVOLUTION_CONF.json：宠物进化的宠物、孵化、骑乘、魔法或相关玩法配置。 结构：RocoDataRows + LocalizationStrings；约 309 条记录；首记录字段：id 、 name 、 allow_show_team_battle_star_array 、 pvp_mute_group 、 evolution_group 、 handbook_evolution_group 、 statistics_evolution_group 、 evolution_chain。
- PET_FEATURE_RAND.json：宠物FEATURE随机池的宠物、孵化、骑乘、魔法或相关玩法配置。 结构：RocoDataRows + LocalizationStrings；约 3 条记录；首记录字段：id 、 feature_rand。
...
- PETBASE_CONF.json：宠物或魔法系统中的PETBASE相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 1015 条记录；首记录字段：id 、 name 、 boss_type 、 move_type 、 completeness 、 pet_evolution_id 、 quality 、 stength_stage。
- PETBASE_USED_BY_FASHION_BOND.json：PETBASEUSED按时装羁绊的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 1015 条记录；首记录字段：id。
- PETFREE_CONF.json：宠物或魔法系统中的PETFREE相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 54 条记录；首记录字段：id 、 petfree_sort 、 level_low 、 level_high 、 reward。
- PETPAGE_BLACKLIST.json：宠物或魔法系统中的PETPAGE黑名单相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；当前为空表；首记录字段：无可用样本字段。
...
- PRIVILEGE_WAND_CONF.json：宠物或魔法系统中的PRIVILEGE魔杖相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 1 条记录；首记录字段：id 、 fashion_item_id 、 magic_transform_id。
...
- BASE_POINT_CONF.json：战斗系统中的基础点数相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 126 条记录；首记录字段：id 、 base_point。
...
- BATTLE_USED_BY_TASK_CONF.json：战斗USED按任务的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 350 条记录；首记录字段：id 、 task_id。
...
- BUFFBASE_CONF.json：战斗系统中的BUFFBASE相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 1879 条记录；首记录字段：id 、 editor_name 、 buffbase_order 、 show_letters 、 client_trigger_type 、 buffbase_param。
...
- ESCAPE_INFO_CONF.json：战斗系统中的逃跑相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；当前为空表；首记录字段：无可用样本字段。
...
- SKILLMACHINE_FILTER_CONF.json：战斗系统中的SKILLMACHINE筛选相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 27 条记录；首记录字段：id 、 filter_type。
- SKILLSPECIAL_CONF.json：战斗系统中的SKILLSPECIAL相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；当前为空表；首记录字段：无可用样本字段。
...
- TERRITORY_TRIAL_CHALLENGE_CONF.json：战斗系统中的TERRITORY试炼挑战相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 2 条记录；首记录字段：id 、 topic 、 battle 、 boss 、 boss_point 、 boss_entry 、 guard。
- TERRITORY_TRIAL_CONF.json：战斗系统中的TERRITORY试炼相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 2 条记录；首记录字段：id 、 topic 、 start_time 、 period 、 redirect 、 challenge_id 、 play_reward_text 、 pet_id。
...
- NRC_AI_FSM_COND_CONF.json：NRCAIFSM条件的 NPC 行为、AI 逻辑或交互流程配置。 结构：RocoDataRows + LocalizationStrings；约 787 条记录；首记录字段：id 、 name 、 fsm_cond。
...
- NRC_AI_PERFORM_POOL_CONF.json：NRCAI表现池的 NPC 行为、AI 逻辑或交互流程配置。 结构：RocoDataRows + LocalizationStrings；约 598 条记录；首记录字段：id 、 name 、 param1 、 pool_number 、 ai_perform_group_1 、 ai_group_param_1 、 ai_perform_group_2 、 ai_group_param_2。
...
- NRC_GROUP_AI_ROLE_TYPE_CONF.json：NRC组AIROLE类型的 NPC 行为、AI 逻辑或交互流程配置。 结构：RocoDataRows + LocalizationStrings；约 137 条记录；首记录字段：id 、 group_ai_role_type_name。
...
- REFRESH_COND_CONF.json：NPC 或 AI 系统中的刷新条件相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 3 条记录；首记录字段：id 、 npcs 、 altitude_min 、 altitude_max 、 depth_min 、 depth_max 、 tiles 、 tags。
...
- SPE_REFRESH_TRIG_CONF.json：NPC 或 AI 系统中的SPE刷新触发相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；当前为空表；首记录字段：无可用样本字段。
...
- MEGAMAP_CLASS_NAME_CONF.json：场景与世界探索中的大地图CLASSNAME相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 10 条记录；首记录字段：id 、 class 、 name。
- MEGAMAP_CONF.json：场景与世界探索中的大地图相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 827 条记录；首记录字段：id 、 genre 、 class 、 display_level_min 、 display_level_max。
- MEGAMAP_GATHERING_CONF.json：场景与世界探索中的大地图GATHERING相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 2595 条记录；首记录字段：id 、 index_method 、 param_id 、 genre。
...
- MEGAMAP_OVERLAP_CONF.json：场景与世界探索中的大地图重叠相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 18 条记录；首记录字段：id 、 res_class_id 、 res_enum_id 、 name 、 icon。
- MEGAMAP_REFRESH_BLACKLIST.json：场景与世界探索中的大地图刷新黑名单相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 1 条记录；首记录字段：id。
- MEGAMAP_SPEED_CONF.json：场景与世界探索中的大地图速度相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 3 条记录；首记录字段：id 、 speed。
...
- SPLINE_CONF.json：SPLINE的音频、动画、模型、特效或资源挂接配置。 结构：RocoDataRows + LocalizationStrings；约 357 条记录；首记录字段：id 、 position 、 rotation 、 scale 、 bp_name 、 spline_point。
...
- TRAVEL_FILTER_CONF.json：场景与世界探索中的旅行筛选相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 41 条记录；首记录字段：id 、 filter_type 、 filter_enum_name 、 filter_enum_value 、 filter_desc 、 filter_icon。
- TRAVEL_SEQUENCE_CONF.json：场景与世界探索中的旅行序列相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 2 条记录；首记录字段：id 、 sequence_default 、 sequence_switch 、 sequence_desc。
...
- DIALOGUE_USED_BY_TASK_CONF.json：对话USED按任务的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 12810 条记录；首记录字段：id 、 task_id。
...
- HOME_USED_BY_TASK_CONF.json：家园USED按任务的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 4 条记录；首记录字段：id 、 task_id。
- MOVIE_USED_BY_TASK_CONF.json：过场USED按任务的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 32 条记录；首记录字段：id 、 task_id。
- OPTION_USED_BY_TASK_CONF.json：选项USED按任务的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 992 条记录；首记录字段：id 、 task_id。
...
- SEQUENCE_USED_BY_TASK_CONF.json：序列USED按任务的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 81 条记录；首记录字段：id 、 task_id。
- STORY_BGM_CONF.json：任务与剧情系统中的剧情BGM相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 8 条记录；首记录字段：id 、 story_bgm_state 、 text。
...
- SUB_TASK_CONF.json：SUB任务的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；当前为空表；首记录字段：无可用样本字段。
...
- TASK_CONF.json：任务的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 3729 条记录；首记录字段：id 、 name 、 task_class 、 paragraph_id 、 task_structure_type 、 task_condition 、 go_guide 、 auto_finish。
- TASK_DATA_GUARD_CONF.json：任务守护的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 37 条记录；首记录字段：id 、 option_id 、 npc_id 、 content_id。
...
- TASK_ITEM.json：任务物品的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 4 条记录；首记录字段：id 、 bag_item_id 、 task_type。
- TASK_MODULE_CONF.json：任务MODULE的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 115 条记录；首记录字段：id 、 task_weight 、 moduel_id。
- TASK_NPC.json：任务NPC的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；当前为空表；首记录字段：无可用样本字段。
- TASK_PET_PARAM_CONF.json：任务宠物PARAM的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 20 条记录；首记录字段：id 、 editor_name 、 task_group。
- TASK_STATE_CONF.json：任务STATE的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 4 条记录；首记录字段：id 、 state_areas 、 begin_tips 、 end_tips 、 state_type 、 effect_begin 、 effect_loop 、 effect_end。
- TASK_STYLE_CONF.json：任务STYLE的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 7 条记录；首记录字段：id 、 icon_open 、 icon_wait 、 icon_done 、 sub_icon_open 、 sub_icon_done 、 minimap_open 、 minimap_wait。
...
- TASK_SWITCH_CONF.json：任务SWITCH的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 11 条记录；首记录字段：id 、 switch_group_id 、 switch_type 、 begintask 、 switch_condition。
- TASK_TOKEN_CONF.json：任务TOKEN的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 9 条记录；首记录字段：id 、 name 、 token_des 、 token__source 、 token_reward。
...
- TEACH_TAB_CONF.json：任务与剧情系统中的TEACH页签相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 5 条记录；首记录字段：id 、 type 、 type_name 、 icon1 、 icon2。
...
- GP_CONTEST_CONF.json：副本或竞技玩法中的GPCONTEST相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 4 条记录；首记录字段：id 、 contest_list。
...
- PVP_MATCH_TIPS_CONF.json：副本或竞技玩法中的PVPMATCHTIPS相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 8 条记录；首记录字段：id 、 pvp_match_tips_text。
...
- PVP_RANK_TRIAL_PET_CONF.json：PVP排位试炼宠物的宠物、孵化、骑乘、魔法或相关玩法配置。 结构：RocoDataRows + LocalizationStrings；约 497 条记录；首记录字段：id 、 baseid 、 pet_info_id 、 level。
- PVP_RANK_TRIAL_PET_LIBRARY_CONF.json：PVP排位试炼宠物LIBRARY的宠物、孵化、骑乘、魔法或相关玩法配置。 结构：RocoDataRows + LocalizationStrings；约 8 条记录；首记录字段：id 、 unit_type 、 pet。
- PVP_RANK_WEEK_TASK_CONF.json：PVP排位WEEK任务的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 5 条记录；首记录字段：id 、 num 、 reward。
...
- BAG_ITEM_TYPE_CONF.json：物品与奖励系统中的背包物品类型相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 22 条记录；首记录字段：id 、 type 、 type_name 、 type_capacity_limit 、 sequence 、 type_hint_limit_max 、 type_number_warn 、 single_item_limit_max。
...
- BOTTLE_VOLUME_CONF.json：物品与奖励系统中的瓶子VOLUME相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 8 条记录；首记录字段：id 、 lower_limit 、 upper_limit 、 exchange_conf。
...
- CALLBACK_MAIL_CONF.json：回调邮件的好友、分享、回流或社交奖励配置。 结构：RocoDataRows + LocalizationStrings；约 6 条记录；首记录字段：id 、 mail_id 、 callback_param_1。
...
- ITEM_LABLE_TYPE_CONF.json：物品与奖励系统中的物品LABLE类型相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 13 条记录；首记录字段：id 、 type_name 、 type_capacity_limit 、 sequence 、 type_number_limit 、 type_hint_limit_max。
...
- MAIL_CONF.json：物品与奖励系统中的邮件相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 101 条记录；首记录字段：id 、 mail_type 、 mail_platform 、 mail_condition 、 validity 、 mail_title 、 mail_sender 、 mail_content。
- OPERATION_MAIL.json：物品与奖励系统中的运营邮件相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；当前为空表；首记录字段：无可用样本字段。
...
- EXCHANGE_NORMAL_FILTER_CONF.json：兑换普通筛选的商品、价格、奖池、刷新或兑换规则配置。 结构：RocoDataRows + LocalizationStrings；约 2 条记录；首记录字段：id 、 filter_type 、 filter_enum_name 、 filter_enum_value 、 filter_desc 、 filter_icon。
...
- MALL_FRAME_CONF.json：商城框架的商品、价格、奖池、刷新或兑换规则配置。 结构：RocoDataRows + LocalizationStrings；约 5 条记录；首记录字段：shop_id 、 tab_id_1 、 tab_name_1 、 icon 、 mall_type 、 system_control_id。
...
- MALL_RAND_CONF.json：商城随机池的商品、价格、奖池、刷新或兑换规则配置。 结构：RocoDataRows + LocalizationStrings；约 1 条记录；首记录字段：id 、 RandItem。
...
- BOND_TAB_CONF.json：羁绊页签的收藏展示、称号、勋章或收集系统配置。 结构：RocoDataRows + LocalizationStrings；约 5 条记录；首记录字段：id 、 fashion_bond_band 、 band_name 、 icon 、 band_long_text 、 band_short_text。
...
- CAMP_CONF.json：营地的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 87 条记录；首记录字段：id 、 editor_name 、 camp_name 、 name 、 advantage_type 、 broadcast_name_func_id。
- CAMP_LEVELUP_CONF.json：营地升级的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 195 条记录；首记录字段：id 、 content_id 、 editor_name 、 level 、 travel_time 、 lay_chance 、 reward_item 、 regional_division。
...
- CLOSET_TAB_CONF.json：衣橱页签的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 22 条记录；首记录字段：id 、 tabname 、 rank_value 、 use_FashionLabelType 、 use_SalonLabelType 、 icon 、 gray_icon 、 shop_icon。
...
- FASHION_PERFORM_CONF.json：时装或套装触发的表现动作、技能演出与待机表现配置。 结构：RocoDataRows + LocalizationStrings；约 31 条记录；首记录字段：id 、 petbase3_id 、 suiteffect3_rest_name 、 suiteffect3_rest_skill 、 suiteffect3_callout_name 、 suiteffect3_callout_skill 、 suiteffect3_win_name 、 suiteffect3_win_skill。
...
- FASHION_TAB_CONF.json：时装页签的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 16 条记录；首记录字段：id 、 use_FashionLabelType 、 icon 、 rank_value。
...
- FURNITURE_CLASSIFICATION_CONF.json：家具分类的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 25 条记录；首记录字段：id 、 tab_name 、 is_first_tab 、 sec_tab_array 、 visual_angle 、 tab_icon_build_1 、 tab_icon_build_2。
...
- GLASS_TYPE_CONF.json：GLASS类型的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 2 条记录；首记录字段：id 、 glass_type 、 weight 、 star_ratio。
...
- HOME_FILTER_CONF.json：家园筛选的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 57 条记录；首记录字段：id 、 filter_type 、 filter_enum_name 、 filter_enum_value 、 filter_desc 、 filter_icon。
...
- MINIGAME_CONF.json：小游戏的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 81 条记录；首记录字段：id 、 editor_name 、 effect_type 、 rule 、 time_limit 、 start_point 、 block_id 、 gameplay_area。
- MINIGAME_RULE_CONF.json：小游戏的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 81 条记录；首记录字段：id 、 group_id 、 contents。
...
- PLANT_TAB_CONF.json：PLANT页签的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 4 条记录；首记录字段：id 、 name。
...
- SALON_TAB_CONF.json：沙龙页签的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 7 条记录；首记录字段：id 、 icon。
...
- TAKE_PHOTO_FILTER_CONF.json：拍照筛选的装扮、家园、家具、种植或生活玩法配置。 结构：RocoDataRows + LocalizationStrings；约 9 条记录；首记录字段：id 、 name 、 icon。
...
- WATER_MARK_CONTROL_CONF.json：水印标记控制的系统参数、平台限制、性能分级或调试配置。 结构：RocoDataRows + LocalizationStrings；约 7 条记录；首记录字段：id 、 world_id 、 namespace 、 close_watermark。
- WATER_MARK_WHITE_LIST_CONF.json：家园或生活玩法中的水印标记白名单相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 1572 条记录；首记录字段：id 、 openid 、 world_id 、 namespace 、 end_time 、 account_type 、 account 、 operator。
...
- COLLEGE_SELECTION_CONF.json：角色成长系统中的学院SELECTION相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 4 条记录；首记录字段：id 、 name 、 pic 、 selected_pic 、 text 、 select_to_dialo 、 editor_name。
- EVOLUTION_ACTION_DATA.json：角色成长系统中的进化动作相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 11 条记录；首记录字段：id 、 action_lower_limit 、 action_upper_limit。
- EVOLUTION_LEVEL_DATA.json：角色成长系统中的进化LEVEL相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 13 条记录；首记录字段：id 、 chance。
...
- OVERRIDE_QUALITY_CONF.json：OVERRIDE品质的系统参数、平台限制、性能分级或调试配置。 结构：RocoDataRows + LocalizationStrings；约 15 条记录；首记录字段：id 、 overrideId 、 name 、 valueType 、 cvar 、 value。
...
- QUALITY_MAPPING_CONF.json：画质映射的系统参数、平台限制、性能分级或调试配置。 结构：RocoDataRows + LocalizationStrings；约 17 条记录；首记录字段：id 、 name 、 valueType 、 Qualities。
...
- ROLE_STAR_NPCLEVEL_CHANGE_CONF.json：角色成长系统中的ROLESTARNPCLEVELCHANGE相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 3116 条记录；首记录字段：id 、 editor_name 、 index_param 、 level 、 down_wave。
...
- HANDBOOK_FILTER_CONF.json：图鉴筛选的图鉴条目、收集顺序或图鉴奖励配置。 结构：RocoDataRows + LocalizationStrings；约 20 条记录；首记录字段：id 、 filter_type 、 filter_enum_name 、 filter_enum_value 、 filter_desc 、 filter_icon。
...
- MEDAL_TASK_CONF.json：勋章任务的任务条件、流程步骤或任务关联配置。 结构：RocoDataRows + LocalizationStrings；约 29 条记录；首记录字段：id 、 desc 、 get_condition 、 condition_data1 、 condition_data2 、 count。
...
- CHAT_EMOJI_CONF.json：CHATEMOJI的界面展示、引导流程或客户端表现配置。 结构：RocoDataRows + LocalizationStrings；约 16 条记录；首记录字段：id 、 emoji_resource_name 、 is_initial_unlock 、 type_desc 、 bottom_description 、 card_quality 、 emoji_use_icon 、 emoji_esc。
- CURVE_DETAIL_INFO.json：客户端表现系统中的曲线DETAIL相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 60 条记录；首记录字段：id 、 curve_id 、 curve_path 、 total_length 、 vector。
- CUSTOMCAMERA_CONF.json：客户端表现系统中的自定义镜头相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 1 条记录；首记录字段：id 、 POS_OFFSET_X 、 POS_OFFSET_Y 、 POS_OFFSET_Z 、 ROT_OFFSET_Y 、 ROT_OFFSET_Z 、 DIST。
...
- MARK_GAMEPLAY_CONF.json：客户端表现系统中的标记GAMEPLAY相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 3 条记录；首记录字段：id 、 gameplay_type 、 priority 、 push_amount 、 push_grid_amount_max 、 push_grid_amount_min 、 create_grid_amount_max 、 create_amount_per_day_max。
- MARK_MESSAGE_LIFE_TIME_CONF.json：标记消息生命周期时间的界面展示、引导流程或客户端表现配置。 结构：RocoDataRows + LocalizationStrings；约 4 条记录；首记录字段：id 、 attitude_count 、 magic_message_add_time。
- MARK_VIDEO_PROTOCOL.json：标记视频PROTOCOL的音频、动画、模型、特效或资源挂接配置。 结构：RocoDataRows + LocalizationStrings；约 37 条记录；首记录字段：id 、 protocol_id 、 protocol_string 、 mask_type 、 tips。
...
- RED_POINT_CONF.json：红点的界面展示、引导流程或客户端表现配置。 结构：RocoDataRows + LocalizationStrings；约 270 条记录；首记录字段：id 、 child_id。
- SECONDARY_TAB_CONF.json：客户端表现系统中的次级页签相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 2 条记录；首记录字段：id 、 name 、 activity_type 、 base_id。
...
- UI_LOBBY_MAIN_COMPASS.json：界面LOBBYMAINCOMPASS的界面展示、引导流程或客户端表现配置。 结构：RocoDataRows + LocalizationStrings；约 19 条记录；首记录字段：id 、 icon_path 、 icon_name 、 icon_name_roco 、 chinese_pinyin。
...
- ACTION_RESULT_TYPE_CONF.json：资源表现系统中的动作结果类型相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 16 条记录；首记录字段：id 、 editor_name 、 expand_dialogs。
...
- MUSIC_APPLY_LIST_CONF.json：音乐应用的音频、动画、模型、特效或资源挂接配置。 结构：RocoDataRows + LocalizationStrings；约 15 条记录；首记录字段：id 、 list_name 、 list_sort 、 list_icon_path 、 list_type 、 interface_type。
...
- MUSIC_TYPE_CONF.json：音乐分类、类型名称与图标展示配置。 结构：RocoDataRows + LocalizationStrings；约 3 条记录；首记录字段：id 、 music_type 、 music_type_name 、 icon_path。
...
- RES_BGS_TIME_CONF.json：背景音资源的播放时长与 Buff 时间映射配置。 结构：RocoDataRows + LocalizationStrings；约 30 条记录；首记录字段：id 、 bgs 、 buff_time。
- RES_BUFF_TIME_CONF.json：Buff 资源的播放时序、前后摇与中断窗口配置。 结构：RocoDataRows + LocalizationStrings；约 235 条记录；首记录字段：id 、 skill_time 、 skill_pre_end 、 skill_counter 、 skill_interrupt 、 skill_melee_before 、 skill_melee_after 、 skill_ranged_before。
- RES_SKILL_TIME_CONF.json：技能资源的播放时序、前后摇与打断窗口配置。 结构：RocoDataRows + LocalizationStrings；约 1377 条记录；首记录字段：id 、 skill_time 、 skill_pre_end 、 skill_counter 、 skill_interrupt 、 skill_melee_after 、 skill_ranged_after。
- SUBTITLE_CONF.json：SUBTITLE的对白文本、字幕、段落或文本表现配置。 结构：RocoDataRows + LocalizationStrings；约 107 条记录；首记录字段：id 、 msg。
- VIDEO_SUBTITLES_CONF.json：视频字幕的音频、动画、模型、特效或资源挂接配置。 结构：RocoDataRows + LocalizationStrings；约 82 条记录；首记录字段：id 、 track_id 、 begin_time 、 end_time 、 content。

### 系统、平台与调试

说明：全局参数、系统开关、平台兼容、性能分级、反作弊与 GM 调试配置。

- ACTCONTROLCONFIG.json：活动开关、审核状态、版本规则与渠道限制配置。 结构：RocoDataRows + LocalizationStrings；约 20 条记录；首记录字段：id 、 editor_name 、 is_open 、 login_plat_limit 、 version_rule 、 channel。
...
- CHANNELCONTROLCONFIG.json：渠道登录入口、版本限制与渠道控制配置。 结构：RocoDataRows + LocalizationStrings；约 8 条记录；首记录字段：id 、 cli_login_channel 、 editor_name。
- CHINA_BENCHMARK_DEVICE_CONF.json：国内设备性能基准、分档评分与硬件型号对照配置。 结构：RocoDataRows + LocalizationStrings；约 2731 条记录；首记录字段：id 、 score 、 benchmarkLevel 、 deviceName 、 cpuModel 、 gpuModel 、 deviceModel。
- CLIENT_PUBLIC_CMD.json：客户端公共命令的系统参数、平台限制、性能分级或调试配置。 结构：RocoDataRows + LocalizationStrings；约 86 条记录；首记录字段：id 、 text。
- FIX_DATA_EVOLUTION_ERROR.json：进化异常数据的修复记录与问题追踪配置。 结构：RocoDataRows + LocalizationStrings；约 275 条记录；首记录字段：id 、 vroleid 、 petbaseid 、 petgid 、 height 、 weight 、 pettalentskill 、 dtEventTime。
...
- FUNCTION_BAN_SCENE_RES_CONF.json：功能封禁期间的场景资源替换、隐藏或降级展示配置。 结构：RocoDataRows + LocalizationStrings；当前为空表；首记录字段：无可用样本字段。
- FUNCTION_BAN_SPECIAL_NPC_CONF.json：功能封禁期间需要特殊处理的 NPC 名单配置。 结构：RocoDataRows + LocalizationStrings；约 2 条记录；首记录字段：id。
- GLOBAL_CONFIG.json：全局配置系统的全局参数、默认值与开关配置。 结构：RocoDataRows + LocalizationStrings；约 479 条记录；首记录字段：id 、 key 、 editor_name。
- GM_BUTTON_CONF.json：GM 调试按钮的显示位置、任务入口与奖励测试配置。 结构：RocoDataRows + LocalizationStrings；约 31 条记录；首记录字段：id 、 editor_name 、 button_string 、 task_id 、 position 、 unlock_camp 、 reward_id 、 world_level。
...
- GM_GROUP_CONF.json：GM 指令分组、页签归属与按钮组织配置。 结构：RocoDataRows + LocalizationStrings；约 1379 条记录；首记录字段：id 、 tab_id 、 button_name 、 gm_group。
- GM_MAINTAB_CONF.json：GM 调试面板主页签、入口名与脚本文件配置。 结构：RocoDataRows + LocalizationStrings；约 49 条记录；首记录字段：id 、 maintab_name 、 lua_filename。
...
- GM_SUBTAB_CONF.json：GM 调试面板子页签与脚本入口配置。 结构：RocoDataRows + LocalizationStrings；约 85 条记录；首记录字段：id 、 subtab_name 、 maintab_id 、 lua_filename。
...
- SYSTEM_RED_POINT_BAN_CONF.json：系统红点的屏蔽条件、禁用范围与控制规则配置。 结构：RocoDataRows + LocalizationStrings；约 29 条记录；首记录字段：id 、 editor_name 、 redpoint_id。
- SYSTEMCONTROLCONFIG.json：系统功能开关、审核状态与版本渠道限制配置。 结构：RocoDataRows + LocalizationStrings；约 215 条记录；首记录字段：id 、 editor_name 、 is_open 、 is_audit 、 login_plat_limit 、 version_rule 、 channel。
- TEST_RECHARGE_AMOUNT_CONF.json：测试充值额度的系统参数、平台限制、性能分级或调试配置。 结构：RocoDataRows + LocalizationStrings；当前为空表；首记录字段：无可用样本字段。
- UNIT_CONF.json：系统层中的单位相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 38 条记录；首记录字段：id 、 title。
- WEIGHT_GROUP_CONF.json：系统层中的权重组相关字段、规则或资源关联。 结构：RocoDataRows + LocalizationStrings；约 2 条记录；首记录字段：id 、 group_weight。
- WORLD_BENCHMARK_DEVICE_CONF.json：国际设备性能基准、分档评分与硬件型号对照配置。 结构：RocoDataRows + LocalizationStrings；约 1442 条记录；首记录字段：id 、 rank 、 score 、 deviceName 、 deviceModel 、 cpuModel 、 gpuModel。